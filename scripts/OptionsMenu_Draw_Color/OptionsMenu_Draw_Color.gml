/// @description  OptionsMenu_Draw_Color(YT)
/// @param YT
function OptionsMenu_Draw_Color() {

	// DEV TOOLS sub-folder draw: COLOR. Rows are actions (open an editor) with no
	...
	_vis_N = 0;
	var _vis_enum = array_create(ClrTools.COUNT, 0);
	for (_i = 0; _i < ClrTools.COUNT; _i++)
	{
	    if (OptionsMenu_option_is_avail(menu_state_COLOR, _i)) { _vis_enum[_vis_N] = _i; _vis_N++; }
	}


	_cursor_vis_pos = 0;
	for (_i = 0; _i < _vis_N; _i++) { if (_vis_enum[_i] == Color_cursor) { _cursor_vis_pos = _i; break; } }
	...
	for (_i = _top_index; _i < _draw_end; _i++)
	{
	    ...
	    _enum = _vis_enum[_i];
	    _text = Color_dg[#_enum,0];

	    if (_enum==Color_cursor) _pi = PI_MENU1;
	    else                     _pi = PI_MENU2;
	    _xl = TextArea1_xl;
	    draw_text_(_xl,_yt1, _text, _font, _pi);


	    if (_enum == Color_cursor)
	    {
